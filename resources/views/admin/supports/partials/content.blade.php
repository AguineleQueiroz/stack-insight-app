@if($supports->getItems())
    <div class="d-block overflow-x-auto pb-5">
        <table class=" my-8 p-8" style="width: 100%;">
            ...
            @foreach ($supports->getItems() as $support)
                <tr class="h-12 odd:bg-slate-50 even:bg-white" id="{{$support->id}}">
                    <td class="px-4 min-w-72">{{ $support->subject}}</td>
                    <td class="text-slate-500 px-4 min-w-72">{{ $support->content_body }}</td>
    {{--                <td class="text-slate-500 px-4">{{ count($support->total_replies) }}</td>--}}
                    <td class="text-slate-500 px-4 min-w-7">
                        <x-replies-support>{{__($support->total_replies)}}</x-replies-support>
                    </td>
                    <td class="px-4 min-w-7">
                        <x-status-support :status="$support->status"></x-status-support>
                    </td>
                    <td class="flex items-center justify-end h-12 px-4 min-w-48">
                        <!-- Settings Dropdown -->
